...
$evidence=Join-Path $PWD 'docs/review/evidence/final-system-3c0d440'
...
foreach($target in $targets){
 ...
 if($api -eq 31){
  ...
  & $adb -s emulator-5554 shell am instrument -w -r -e class com.personal.cameraalarm.AlarmStopInstrumentedTest com.personal.cameraalarm.test/androidx.test.runner.AndroidJUnitRunner > "$evidence/api31-warm-stop.txt"
 }
