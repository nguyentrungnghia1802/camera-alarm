...
class DataStoreAlarmStateStore(
    context: Context,
    private val dataStore: DataStore<Preferences> = context.runtimeDataStore,
    private val bootCount: () -> Int = { Settings.Global.getInt(context.contentResolver, Settings.Global.BOOT_COUNT, 0) },
    private val processNonce: String = UUID.randomUUID().toString()
) : AlarmStateStore {
    override var restoreNote: String? = null
        private set
    override suspend fun read(): AlarmState {
        // Check and clear the CURRENT preferences inside one transaction; never clear from an old snapshot.
        val p = dataStore.edit { current ->
            val boot = bootCount()
            if (current[BOOT] != boot || (current[STATE] == "ringing" && current[OWNER_NONCE] != processNonce)) {
                restoreNote = "previous=${current[STATE]} previous_boot=${current[BOOT]} boot=$boot invalidated=true"
                current.clear()
                current[BOOT] = boot
                current[STATE] = "idle"
            }
        ...
        return when (p[STATE]) {
            "pending" -> snapshot(p)?.let {
                AlarmState.Pending(it, p[AT] ?: return AlarmState.Idle, p[ELAPSED_DEADLINE], p[ATTEMPT] ?: 0)
            } ?: AlarmState.Idle
            "ringing" -> snapshot(p)?.let { AlarmState.Ringing(it, p[AT] ?: return AlarmState.Idle) } ?: AlarmState.Idle
    ...
    override suspend fun write(state: AlarmState) {
        dataStore.edit { p ->
            p.clear(); p[BOOT] = bootCount()
            when (state) {
                AlarmState.Idle -> p[STATE] = "idle"
                is AlarmState.Pending -> {
                    p[STATE] = "pending"; p[AT] = state.scheduledAtEpochMs; putSnapshot(p, state.trigger)
                    state.deadlineElapsedMs?.let { p[ELAPSED_DEADLINE] = it }
                    p[ATTEMPT] = state.recoveryAttempt
                }
    ...
    private fun snapshot(p: Preferences): TriggerSnapshot? {
        ...
        return TriggerSnapshot(AlarmToken(token), source, key, rule, p[TITLE], p[PREVIEW], received, p[DEADLINE], p[DELAY])
    }
    private fun putSnapshot(p: MutablePreferences, t: TriggerSnapshot) {
        ...
        t.deadlineEpochMs?.let { p[DEADLINE] = it }; t.configuredDelayMs?.let { p[DELAY] = it }
        t.title?.let { p[TITLE] = it.take(300) }; t.textPreview?.let { p[PREVIEW] = it.take(300) }
    }
    companion object {
        private val ELAPSED_DEADLINE = longPreferencesKey("elapsed_deadline")
        private val ATTEMPT = intPreferencesKey("recovery_attempt")
        private val DEADLINE = longPreferencesKey("deadline")
        private val DELAY = longPreferencesKey("configured_delay")
