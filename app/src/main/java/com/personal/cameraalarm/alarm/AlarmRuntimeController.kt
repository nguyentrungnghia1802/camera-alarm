...
class AlarmRuntimeController(private val player: AlarmPlayer, private val vibration: VibrationController) {
    var activeToken: AlarmToken? = null
        private set
    fun start(token: AlarmToken, vibrationEnabled: Boolean): List<String> {
        if (activeToken != null) return emptyList()
        activeToken = token
        val errors = mutableListOf<String>()
        player.start().exceptionOrNull()?.let { errors += "audio: ${it.message ?: it.javaClass.simpleName}" }
        if (vibrationEnabled) try { vibration.startRepeating() } catch (e: RuntimeException) {
    ...
    fun stop(token: AlarmToken?): List<String> {
        ...
        activeToken = null
        val errors = mutableListOf<String>()
        try { player.stop() } catch (e: RuntimeException) { errors += "audio stop: ${e.message ?: e.javaClass.simpleName}" }
        try { vibration.stop() } catch (e: RuntimeException) { errors += "vibration stop: ${e.message ?: e.javaClass.simpleName}" }
        return errors
    }
}
